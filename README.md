...
- Deux notes sont jouées au piano (échantillons Salamander Grand Piano via [Tone.js](https://tonejs.github.io/), avec repli synthétisé si les échantillons ne se chargent pas) ; il faut cliquer sur l'intervalle correspondant.
- **Progression** : on commence avec deux intervalles simples (quinte juste et octave). Un nouvel intervalle se débloque quand tous les intervalles en cours sont bien maîtrisés (≥ 80 % de réussite récente). La progression (intervalles débloqués, statistiques, probabilité d'apparition) est suivie **séparément pour le mode ascendant et descendant**.
